...
class LineItemTest < ActiveSupport::TestCase
  ...
  test "should respond to buyable" do
    line_item=LineItem.new
    assert_respond_to line_item, :buyable
  end
  ...
  test "should not save without buyable" do
    line_item=LineItem.new(valid.except(:buyable))
  ...
  test "should copy_cost_from_product" do
    ...
    assert_equal line_item.buyable.currency, line_item.currency
    assert_equal line_item.buyable.cost, line_item.cost
  end
  ...
  test "cost should equal unit_cost*quantity" do
    ...
    line_item.save
    assert_equal line_item.buyable.unit_cost, line_item.unit_cost
    assert_equal line_item.buyable.cost*line_item.quantity, line_item.cost
  end
  ...
  test "should remove stock allocation on destroy" do
    ...
    stock=line_item.buyable.stock_levels.new(due_at: 5.minutes.ago, start_quantity: 10)
    assert stock.save
    line_item.reload
  ...
  test "should split across stock levels when available" do
    ...
    assert line_item.save
    stock=line_item.buyable.stock_levels.new(due_at: 2.days.ago, start_quantity: 5)
    assert stock.save
    stock2=line_item.buyable.stock_levels.new(due_at: 1.day.ago, start_quantity: 10)
    assert stock2.save
    stock3=line_item.buyable.stock_levels.new(due_at: 5.minutes.ago, start_quantity: 8)
    assert stock3.save
    stock4=line_item.buyable.stock_levels.new(due_at: 5.minutes.ago, start_quantity: 5)
    assert stock4.save
    line_item.reload
    assert_equal 4, line_item.buyable.stock_levels.count
  ...
  test "should be weight of product times quantity" do
    ...
    line_item=LineItem.new(valid.merge(buyable: product, quantity: 20))
  ...
  test "should set type as Product when adding sub_product" do
    sub_product=products(:sub_product)
    line_item=LineItem.new(valid.merge(buyable: sub_product))
    line_item.save
    assert_equal "Product", line_item.buyable_type
  end

  test "should allow adding gift_card as buyable" do
    gift_card=gift_cards(:ten_pounds)
    line_item=LineItem.new(valid.merge(buyable: gift_card))
    assert line_item.save
